...
<div class="w-full {$$props.class}">
    ...
    <div class="relative w-full">
        {#if $$slots.start}
            <div class="absolute inset-y-0 left-0 z-5 pl-2 flex items-center text-gray-400">
                <slot name="start"/>
            </div>
        {/if}
        <slot {classes}/>
        {#if $$slots.end}
            <div class="absolute inset-y-0 right-0 z-5 pr-2 flex items-center text-gray-400">
                <slot name="end"/>
            </div>
        {/if}
    </div>
